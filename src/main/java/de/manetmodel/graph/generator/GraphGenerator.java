...
public class GraphGenerator<V extends Vertex, E extends Edge> {
	...
	while ((currentVertex.x() <= playground.width.max - playground.edgeDistance.max)
		&& vertexCount < playground.vertexCount.max) {
	    if (vertexCount > 1) {
	    ...
	    while (currentVertex.y() <= (playground.height.max - playground.edgeDistance.max)
		    && vertexCount < playground.vertexCount.max) {
